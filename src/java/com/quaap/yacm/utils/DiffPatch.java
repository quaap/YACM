...
/* Modified by kliether 2009-03-05 */
...
public class DiffPatch {
         ...
         if (doubleEnd) {
            ...
            for (int k = -d; k <= d; k += 2) {
               ...
               if (done) {
                  ...
                  LinkedList<Diff> a = diff_path1(v_map1, text1.substring(0, text1.length() - x),
         ...
         while (true) {
            if (v_map.get(d).contains(diff_footprint(x - 1, y))) {
               ...
               if (last_op == Operation.DELETE) {
                  path.getFirst().text = text1.charAt(x) + path.getFirst().getText();
               } else {
               ...
               if (last_op == Operation.INSERT) {
                  path.getFirst().text = text2.charAt(y) + path.getFirst().getText();
               } else {
   ...
   public void diff_cleanupSemantic(LinkedList<Diff> diffs) {
      ...
      while (thisDiff != null) {
         if (thisDiff.getOperation() == Operation.EQUAL) {
            ...
            length_changes2 += thisDiff.getText().length();
               ...
               if (count_delete != 0 || count_insert != 0) {
                  ...
                  if (count_delete != 0 && count_insert != 0) {
                     ...
                     if (commonlength != 0) {
                        ...
                        thisDiff.text = text_insert.substring(text_insert.length() - commonlength) + thisDiff.getText();
                  ...
                  prevEqual.text += thisDiff.getText();
                  pointer.remove();
                  thisDiff = pointer.previous();
      ...
      while (nextDiff != null) {
         if (prevDiff.getOperation() == Operation.EQUAL &&
            ...
            if (thisDiff.getText().endsWith(prevDiff.getText())) {
               ...
               thisDiff.text = prevDiff.getText() + thisDiff.getText().substring(0, thisDiff.getText().length() - prevDiff.getText().length());
               nextDiff.text = prevDiff.getText() + nextDiff.getText();
               pointer.previous(); // Walk past nextDiff.
               pointer.previous(); // Walk past thisDiff.
               pointer.previous(); // Walk past prevDiff.
               pointer.remove(); // Delete prevDiff.
               ...
               prevDiff.text += nextDiff.getText();
               thisDiff.text = thisDiff.getText().substring(nextDiff.getText().length()) + nextDiff.getText();
               pointer.remove(); // Delete nextDiff.
               nextDiff = pointer.hasNext() ? pointer.next() : null;
               changes = true;
            }
   ...
   public String diff_toDelta(LinkedList<Diff> diffs) {
      ...
      for (Diff aDiff : diffs) {
         switch (aDiff.getOperation()) {
            case INSERT:
               text.append("+").append(encode(aDiff.getText())).append("\t");
               break;
            case DELETE:
      ...
      for (int d = 0; d < pattern.length(); d++) {
         ...
         for (int j = finish - 1; j >= start; j--) {
            if (d == 0) {
               ...
               rd[j] = ((rd[j + 1] << 1) | 1) & (s.containsKey(text.charAt(j))
                       ? s.get(text.charAt(j)) : 0) | ((last_rd[j + 1] << 1) | 1) | ((last_rd[j] << 1) | 1) | last_rd[j + 1];
            }
            if ((rd[j] & matchmask) != 0) {
   ...
   private double match_bitapScore(int e, int x, int loc,
      ...
      return (e / (float) pattern.length() / Match_Balance) + (d / (float) score_text_length / (1.0 - Match_Balance));
   }
   ...
   public LinkedList<Patch> patch_make(String text1, LinkedList<Diff> diffs) {
      ...
      for (Diff aDiff : diffs) {
         if (patch.diffs.isEmpty() && aDiff.getOperation() != Operation.EQUAL) {
         ...
         switch (aDiff.getOperation()) {
            case INSERT:
               ...
               postpatch_text = postpatch_text.substring(0, char_count2) + aDiff.getText() + postpatch_text.substring(char_count2);
               break;
            case DELETE:
               patch.length1 += aDiff.getText().length();
               patch.diffs.add(aDiff);
               postpatch_text = postpatch_text.substring(0, char_count2) + postpatch_text.substring(char_count2 + aDiff.getText().length());
               break;
            case EQUAL:
            ...
            if (text1.equals(text2)) {
               ...
               for (Diff aDiff : aPatch.diffs) {
                  if (aDiff.getOperation() != Operation.EQUAL) {
                     ...
                     if (aDiff.getOperation() == Operation.INSERT) {
                        // Insertion
                        text = text.substring(0, start_loc + index2) + aDiff.getText() + text.substring(start_loc + index2);
   ...
   protected String patch_addPadding(LinkedList<Patch> patches) {
      ...
      } else if (nullPadding.length() > diffs.getFirst().getText().length()) {
         // Grow first equality.
         Diff firstDiff = diffs.getFirst();
         int extraLength = nullPadding.length() - firstDiff.getText().length();
         firstDiff.text = nullPadding.substring(firstDiff.getText().length()) + firstDiff.getText();
      ...
      if (diffs.isEmpty() || diffs.getLast().getOperation() != Operation.EQUAL) {
         ...
         patch.length1 += nullPadding.length();
         patch.length2 += nullPadding.length();
      } else if (nullPadding.length() > diffs.getLast().getText().length()) {
         // Grow last equality.
         Diff lastDiff = diffs.getLast();
         int extraLength = nullPadding.length() - lastDiff.getText().length();
      ...
      while (bigpatch != null) {
         ...
         while (!bigpatch.diffs.isEmpty()) {
            ...
            while (!bigpatch.diffs.isEmpty() && patch.length1 < patch_size - Patch_Margin) {
               ...
               if (diff_type == Operation.INSERT) {
                  ...
                  if (diff_text.equals(bigpatch.diffs.getFirst().getText())) {
                     ...
                     bigpatch.diffs.getFirst().text = bigpatch.diffs.getFirst().getText().substring(diff_text.length());
                  }
               }
            }
            ...
            if (postcontext.length() != 0) {
               ...
               if (!patch.diffs.isEmpty() && patch.diffs.getLast().getOperation() == Operation.EQUAL) {
   ...
   public static class Diff {
      ...
      public String toString() {
         String prettyText = this.getText().replace('\n', '\u00b6');
         return "Diff(" + this.getOperation() + ",\"" + prettyText + "\")";
      }
      ...
      public boolean equals(Object d) {
         if (d instanceof Diff) {
            return (((Diff) d).getOperation() == this.getOperation()) && (((Diff) d).getText().equals(this.getText()));
         }
         return false;
      }
      ...
      public int hashCode() {
         int hash = 3;
         hash = 73 * hash + (this.operation != null ? this.operation.hashCode() : 0);
   ...
   public static class Patch {
      ...
      public String toString() {
         ...
         for (Diff aDiff : this.diffs) {
            ...
            text.append(encode(aDiff.getText())).append("\n");
         }
...
//   }
   ...
   private static String decode(String text) {
      ...
      try {
         ...
         throw new IllegalArgumentException(
                 "Illegal escape in diff_fromDelta: " + text, e);
      }
